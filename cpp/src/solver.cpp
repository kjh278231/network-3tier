...
Solver::Solver(const ProblemInstance& instance, const SolverConfig& config)
    ...
    // build_cost_matrices() should be called by the user before passing instance
    // or we can call it here, but let's ensure it's safe.
    // inst_ is a copy, so it's safe to modify.
...
SolverResult Solver::solve_fixed(const std::vector<int>& forced_open_warehouses) {
    ...
    for (int c = 0; c < nc; ++c) {
        ...
        if (mapped >= 0 && mapped < nw && sol.open_warehouses[mapped]) {
    ...
    for (int iter = 0; iter < fixed_config.max_iterations; ++iter) {
        ...
        if (candidate.is_feasible && 
            (candidate.total_inbound > best.total_inbound || 
             (candidate.total_inbound == best.total_inbound && candidate.total_cost < best.total_cost))) {
            best = candidate;
        }
    }
